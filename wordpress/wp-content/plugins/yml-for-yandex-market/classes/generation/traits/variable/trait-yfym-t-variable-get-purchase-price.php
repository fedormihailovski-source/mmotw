<?php
/**
 * Traits Purchase Price for variable products
 *
...
trait YFYM_T_Variable_Get_Purchase_Price {
	/**
	...
	public function get_purchase_price( $tag_name = 'purchase_price', $result_xml = '' ) {
		...
		$tag_value = apply_filters(
			'y4ym_f_variable_tag_value_purchase_price',
			$tag_value,
			[ 
		...
		if ( ! empty( $tag_value ) ) {
			$tag_name = apply_filters(
				'y4ym_f_variable_tag_name_purchase_price',
				$tag_name,
				[ 
					'product' => $this->get_product(),
					'offer' => $this->get_offer()
		...
		$result_xml = apply_filters(
			'y4ym_f_variable_tag_purchase_price',
			$result_xml,
			[ 
				'product' => $this->get_product(),
				'offer' => $this->get_offer()
