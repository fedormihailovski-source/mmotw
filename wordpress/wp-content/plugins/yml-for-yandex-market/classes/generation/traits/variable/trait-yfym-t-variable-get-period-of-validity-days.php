...
trait YFYM_T_Variable_Get_Period_Of_Validity_Days {
	...
	public function get_period_of_validity_days( $tag_name = 'period-of-validity-days', $result_xml = '' ) {
		...
		if ( empty( $period_of_validity_days ) || $period_of_validity_days === 'disabled' ) {
			...
			$tag_value = $this->get_offer()->get_attribute( wc_attribute_taxonomy_name_by_id( $period_of_validity_days ) );
			if ( empty( $tag_value ) ) {
				$tag_value = $this->get_product()->get_attribute( wc_attribute_taxonomy_name_by_id( $period_of_validity_days ) );
			}
		}
		$tag_value = apply_filters(
			'y4ym_f_variable_tag_value_period_of_validity_days',
			$tag_value,
			[ 
		...
		if ( ! empty( $tag_value ) ) {
			$tag_name = apply_filters(
				'y4ym_f_variable_tag_name_period_of_validity_days',
				$tag_name,
				[ 
					'product' => $this->get_product(),
					'offer' => $this->get_offer()
		...
		$result_xml = apply_filters(
			'y4ym_f_variable_tag_period_of_validity_days',
			$result_xml,
			[ 
				'product' => $this->get_product(),
				'offer' => $this->get_offer()
