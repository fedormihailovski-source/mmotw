...
trait YFYM_T_Variable_Get_Country_Of_Orgin {
	...
	public function get_country_of_origin( $tag_name = 'country_of_origin', $result_xml = '' ) {
		...
		if ( empty( $country_of_origin ) || $country_of_origin === 'disabled' ) {
			...
			$tag_value = $this->get_offer()->get_attribute( wc_attribute_taxonomy_name_by_id( $country_of_origin ) );
			if ( empty( $tag_value ) ) {
				$tag_value = $this->get_product()->get_attribute( wc_attribute_taxonomy_name_by_id( $country_of_origin ) );
			}
		}
		$tag_value = apply_filters(
			'y4ym_f_variable_tag_value_country_of_origin',
			$tag_value,
			[ 
		...
		if ( ! empty( $tag_value ) ) {
			$tag_name = apply_filters(
				'y4ym_f_variable_tag_name_country_of_origin',
				$tag_name,
				[ 
					'product' => $this->get_product(),
					'offer' => $this->get_offer()
		...
		$result_xml = apply_filters(
			'y4ym_f_variable_tag_country_of_origin',
			$result_xml,
			[ 
				'product' => $this->get_product(),
				'offer' => $this->get_offer()
