...
trait YFYM_T_Variable_Get_Vendor {
	...
	public function get_vendor( $tag_name = 'vendor', $result_xml = '' ) {
		...
		if ( ( is_plugin_active( 'perfect-woocommerce-brands/perfect-woocommerce-brands.php' )
			...
			if ( $vendor !== 'disabled' ) {
				$vendor = (int) $vendor;
				$vendor_yml = $this->get_offer()->get_attribute( wc_attribute_taxonomy_name_by_id( $vendor ) );
				if ( ! empty( $vendor_yml ) ) {
		...
		if ( $skip_vendor_reason !== false ) {
			$this->add_skip_reason( [ 
				...
				'file' => 'trait-yfym-t-variable-get-vendor.php',
				'line' => __LINE__
			] );
			return '';
		}
		...
		$vendor_name = apply_filters(
			'y4ym_f_variable_tag_value_vendor',
			$vendor_name,
			[ 
				'product' => $this->get_product(),
				'offer' => $this->get_offer()
		...
		if ( ! empty( $vendor_name ) ) {
			$tag_name = apply_filters(
				'y4ym_f_variable_tag_name_vendor',
				$tag_name,
				[ 
					'product' => $this->get_product(),
					'offer' => $this->get_offer()
